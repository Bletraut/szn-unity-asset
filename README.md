...
1. Откройте меню сборки проекта, выберите текущую платформу `WebGL` и зайдите в меню настройки сборки проекта `(File -> Build Settings -> Player Settings)`. В разделе `Resolution and Presentation` выберите `SznTemplate`.
